...
| Social Login (Kakao, Google) | 다수의 인원이 펀딩에 참여함에 따라 사용자의 접근성을 고려하여 카카오 로그인과 구글 로그인 기능 구현을 결정했습니다 |
| Kakaopay 온라인 결제 API | 후원 결제 내역을 수집하기 위해 결제 기능 구현이 필요했고, 이에 카카오페이 결제 기능을 구현해 사용자에게 보다 보증된 결제 서비스를 제공하고자 했습니다. |
